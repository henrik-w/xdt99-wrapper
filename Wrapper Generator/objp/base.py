...
from util import pyref, instance, nspoint, nssize, nsrect
...
TYPE_SPECS = [
    ...
    TypeSpec(instance, 'instancetype', 'ObjP_obj_o2p(%s)', 'ObjP_obj_p2o(%s)'),
    TypeSpec(str, 'NSString *', 'ObjP_str_o2p(%s)', 'ObjP_str_p2o(%s)'),
    TypeSpec(tuple, 'NSArray *', 'ObjP_list_o2p(%s)', 'ObjP_list_p2o(%s)'),
    TypeSpec(list, 'NSMutableArray *', 'ObjP_list_o2p(%s)', 'ObjP_list_p2o(%s)'),
    TypeSpec(dict, 'NSMutableDictionary *', 'ObjP_dict_o2p(%s)', 'ObjP_dict_p2o(%s)'),
...
def tmpl_replace(tmpl, **replacments):
    ...
    for placeholder, replacement in replacments.items():
        result = result.replace('%%{}%%'.format(placeholder), replacement.strip())
    return result
...
def get_objc_signature(methodspec):
    ...
    #print("\nElementnamen: %s\nArgument-Spec:%s" % (name_elems, methodspec.argspecs))
